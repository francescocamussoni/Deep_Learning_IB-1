...
def fit(size_bacht=50, lr=1e-5, landa=1e-2, epochs=200, loss_f=mse, grad_f=grad_MSE,
    ...
    W1 = np.random.uniform(-1,1, size=(dim+1, n_neuronas)) * 1e-1
    W2 = np.random.uniform(-1,1, size=(n_neuronas+1, clases)) * 1e-1
...
# fit(epochs=200, lr=1e-3, landa=1e-2, act_1=Relu, grad_1=gradRelu, act_2=sigmoid, grad_2=grad_sigmoid)

# fit(epochs=200, lr=1e-2, landa=1e-4,  loss_f=lossSoftmax, grad_f=grad_Softmax,
#                 act_1=Relu, grad_1=gradRelu, act_2=sigmoid, grad_2=grad_sigmoid)      # De Guate

fit(epochs=200, lr=5e-3, landa=1e-2,  loss_f=lossSoftmax, grad_f=grad_Softmax,
                act_1=Relu, grad_1=gradRelu, act_2=sigmoid, grad_2=grad_sigmoid)        # Este se dio bien, pero se puede mejorar
